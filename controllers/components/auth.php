 ...
 * @access public
 */
	var $userScope = array('User.active' => 1);//array();
 ...
 * @access public
 */
	var $actionPath = 'controllers/';//null;
 ...
 * @access public
 */
	var $loginAction = array('controller' => 'users', 'action' => 'login', 'plugin' => 'user', 'admin' => false);//null;
 ...
 * @access public
 */
	var $loginError = 'Invalid username/password';//null;
 ...
 * @access public
 */
	var $authError = 'Not authorized';//null;
 ...
 */
	var $allowedActions = array('display', 'register', 'login', 'logout');//array();
	...
	function initialize(&$controller, $settings = array()) {
		...
		if (!empty($prefixes)) {
			foreach ($prefixes as $prefix) {
				$this->actionMap = array_merge(array(
					$prefix . '_index' => 'read',
					$prefix . '_add' => 'create',
					$prefix . '_edit' => 'update',
					$prefix . '_view' => 'read',
 ...
 */
	function isAuthorized($type = null, $object = null, $user = null) {
		...
		switch ($type) {
			...
			break;
			case 'crud':
				$this->mapActions();//useless when no $map is passed, initialize will map actions already
				if (!isset($this->actionMap[$this->params['action']])) {
					...
					$valid = $this->Acl->check(
						$user,
						$this->action(':plugin/:controller'),
						$this->actionMap[$this->params['action']]
					);
				}
			break;
			case 'model':
				$this->mapActions();//useless when no $map is passed, see 'crud'
			...
			case 'acl':
				...
				if (!isset($this->actionMap[$this->params['action']])) {
					...
					if (empty($object->id)) {
						return $this->isAuthorized('crud', null, $user);
					}
					$acoNode = $object->node(null, 'Aco');
					$valid = $this->Acl->check(
						$user,
						$acoNode[0]['Aco'],
						$this->actionMap[$this->params['action']]
